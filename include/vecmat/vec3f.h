...
VECMAT_ALIGN_WARN_SUPPRESS
struct VECMAT_ALIGN vec3f: public c::vec3f {
    VECMAT_INLINE vec3f()
    {
    }

    VECMAT_INLINE vec3f(const c::vec3f v) : c::vec3f{v.x, v.y, v.z}
    {
    }

    VECMAT_INLINE vec3f(float x, float y, float z) : c::vec3f{x, y, z}
    {
    }
